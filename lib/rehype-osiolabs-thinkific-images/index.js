  ...
  function transformer(tree, file) {
    return new Promise(async (resolve) => {
      ...
      const visitor = function replaceImagePaths(node) {
        ...
        if (node.tagName === 'img' && node.properties.src.charAt(0) === '/') {
          if (!fs.existsSync(`${PWD}${node.properties.src}`)) {
            file.message(`Invalid image path ${node.properties.src}`, node, 'osiolabs:thinkific-image');
          }
          else {
      ...
      for (const node of nodesToChange) {
        const newSrc = 'https://drupalize.me/sites/default/files/tutorials/' + path.basename(node.properties.src);
        const results = await checkLinks([newSrc]);
        if (results[newSrc].status == 'alive') {
          file.info(`Updated image src from ${node.properties.src} to ${newSrc}`, node, 'osiolabs:thinkific-image');
          node.properties.src = newSrc;
          node.properties.class = ['fr-fic fr-dii'];
        }
        else {
          file.message(`Invalid URL generated for <img> tag: ${newSrc}`, node, 'osiolabs:thinkific-image');
        }
      }
