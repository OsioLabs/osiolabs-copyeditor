    ...
    return new Promise(async (resolve) => {
      ...
      const visitor = function replaceImagePaths(node) {
        ...
        if (
          ...
          node.properties.href.charAt(0) === '/'
        ) {
          if (!fs.existsSync(`${PWD}${node.properties.href}`)) {
            file.message(
              `Invalid tutorial path ${node.properties.src}`,
              node,
              'osiolabs:thinkific-links'
            );
          } else {
      ...
      for (const node of nodesToChange) {
        ...
        promises.push(
          fs.promises
            .readFile(`${PWD}${linkedTutorial}`, 'utf-8')
            .then((data) => {
              ...
              if (url && url[1]) {
                ...
                file.info(
                  `Replaced link to ${linkedTutorial}`,
                  node,
                  'osiolabs:thinkific-links'
                );
              } else {
                ...
                node.tagName = 'span';
                file.info(
                  `Removed link to ${linkedTutorial}`,
                  node,
                  'osiolabs:thinkific-links'
                );
              }
